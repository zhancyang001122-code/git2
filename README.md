...
默认使用本地演示数据。ArchFlow 已预设接口地址、模型名称和输出规格，访问者只需点击左下角“方案一组”或“API Key 配置”，分别填写并应用语言大模型与生图大模型两把 API Key。当前默认连接为：

- 语言大模型：阿里云百炼兼容接口，模型 ID `6736696`；
- 生图大模型：第三方 NewAPI 接口，默认模型 `gpt-image-2`，应用 Key 时会读取可用模型并自动识别 OpenAI / Gemini 生图模型；
- 也可切换 OpenAI 官方、Gemini 官方或自定义兼容服务。

每个 Key 都有独立“应用 Key”按钮。语言模型通过一次最小对话验证 Key 与模型 ID，生图模型通过模型列表验证鉴权和可见性；两项均显示“已连接”后才会启用真实接口。连接后：
