...
test('生产巡检会隔离三路生图、跳过未配置槽位并把供应商瞬态失败降级', () => {
  ...
  assert.match(script, /const CANARY_JPEG_BASE64 = /)
  const jpegMatch = script.match(/const CANARY_JPEG_BASE64 = '([^']+)'/)
  assert.ok(jpegMatch)
  const jpegBytes = Buffer.from(jpegMatch[1], 'base64')
  assert.deepEqual([...jpegBytes.subarray(0, 3)], [0xff, 0xd8, 0xff])
  assert.ok(jpegBytes.length > 1024)
  assert.doesNotMatch(script, /iVBORw0KGgoAAAANSUhEUgAAAAEAAAAB/)
  assert.match(script, /name: 'archflow-canary\.jpg'/)
  assert.match(script, /mimeType: 'image\/jpeg'/)
  assert.match(script, /data: CANARY_JPEG_BASE64/)
  assert.doesNotMatch(script, /data: CANARY_PNG_BASE64/)
  ...
  assert.match(script, /value \|\| '1024x1024'/)
  assert.match(script, /'4K': '3840x2160'/)
