...
test('第二路 NewAPI 使用独立密钥、真实模型和 Gemini 原生图生图协议', () => {
  ...
  assert.match(source, /responseMode: 'url',[\s\S]*?size: '4K'/)
...
test('长耗时 4K 请求转为服务端后台任务并由前端轮询', () => {
  ...
  assert.match(source, /storeManagedImageOutput\(taskId, userId, imageUrl, taskDeadline\)/)
  assert.match(source, /storage\/v1\/object\/sign/)
  assert.match(source, /image_url: durableImageUrl/)
  assert.match(source, /MANAGED_TASK_READ_MAX_ATTEMPTS = 3/)
  assert.match(source, /cleanupExpiredManagedImageTasks\(expiredBefore\)/)
  assert.match(source, /managed_image_cleanup_failed/)
...
test('第二供应商的 HTTP 或 HTTPS 成图会验真后持久化为私有 Storage 地址', () => {
  ...
  assert.match(source, /REMOTE_IMAGE_MAX_REDIRECTS = 3/)
  assert.match(source, /const remoteImage = inlineImage \? null : await downloadRemoteImage\(imageUrl, taskDeadline\)/)
  assert.match(source, /boundedTaskTimeout\(taskDeadline, 30_000\)/)
  assert.doesNotMatch(source, /if \(\/\^https:\\\/\\\/\/i\.test\(imageUrl\)\) return imageUrl/)
})
